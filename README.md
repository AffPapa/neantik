...
Публичный runtime `153.0.8010.36` ниже текущего проверяемого security baseline
репозитория `153.0.8010.52`; подпись и нотарификация не заменяют обновление
исправлений безопасности. Новые изменения в этой ветке не являются новым
бинарным релизом, пока для них не опубликованы отдельные подписанные и
нотарифицированные DMG/ZIP в Releases. Кандидат `0.7.4 (67)` пока проходит
source-pair, runtime и release-проверки и не заменяет опубликованный `0.7.3`.
...
## В локальном кандидате 0.7.4 (67)
...
Публичный `0.7.3` опубликован для public-alpha изоляции профилей. Строгая
