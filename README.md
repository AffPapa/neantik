...
Последний опубликованный Direct-релиз — NeAntik `0.7.8` (build `71`).
Рабочее дерево готовит manager-only кандидат `0.7.9` (build `72`); пока все
...
- текущие публичные ZIP/DMG и SHA-256 указаны в [релизе 0.7.8](https://github.com/AffPapa/neantik/releases/tag/v0.7.8).

Для кандидата `0.7.9` Chromium сохраняется без пересборки. Предыдущий
публичный релиз `0.7.8` остаётся rollback-точкой. Новый бинарник публикуется
