...
- Главная панель оставляет только запуск и изменение, а данные и удаление
  перенесены в компактное меню.
- Добавлены закрепление, архив без удаления BrowserData и безопасное
  ...
  папкой. Настройка прокси и её пароль копируются транзакционно, пароль
  остаётся только в Связке ключей.
- Добавлено массовое создание до 100 независимых профилей из списка прокси.
  ...
  метаданные и Keychain сохраняются атомарно: ошибка полностью откатывает
  импорт. Даже при редком повторном сбое Связки ключей осиротевший секрет
  помечается для безопасной одноразовой очистки при следующем запуске.
