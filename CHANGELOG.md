## Direct 0.7.3 (66) — кандидат, не опубликован

- Перед подписанием менеджера удаляется отладочная таблица путей к локальным исходникам и объектным файлам сборки. Подписанные опубликованные архивы не изменяются на месте.
- Секретов в ходе расширенной проверки не подтверждено; исправление касается раскрытия внутренних путей, а не отзыва обнаруженных ключей.
- Chromium остаётся 153.0.8010.36: разрыв с исправлениями безопасности .52/.53 сохраняется. Старые выпуски и сторонние копии этим обновлением не очищаются.

## Direct 0.7.2 (65) — опубликован

- Восстановление снимка резервирует состояние до замены данных. Если процесс завершается до подтверждения версии, запуск блокируется до восстановления совместимого снимка.
- Снимки без подтверждённой версии отклоняются в интерфейсе до изменения текущих данных. При повторной попытке неопределённая версия не приписывается резервной копии.
- Исправлено сравнение эквивалентных путей macOS при проверке принадлежности снимка; проверки владельца и символических ссылок сохранены.
- Добавлены проверки реального аварийного завершения отдельного тестового процесса после замены файлов, копирования без пароля, отдельного согласия на перенос пароля и отката при ошибке записи. Проверки используют синтетические данные; миграция баз Chromium между версиями и сбой питания ими не подтверждаются.
- Сборка локальной Dev-оболочки ограничена двумя заданиями. Chromium 153.0.8010.36 не пересобирался и не актуален по исправлениям безопасности .52/.53.
- Новая защита незавершённого восстановления действует в этой версии менеджера; старые выпуски не понимают новый маркер. Не используйте старый менеджер для обхода блокировки восстановления.

## Direct 0.7.1 (64) — опубликован

- Обычная остановка рабочего места запрашивает штатное закрытие точного приложения macOS вместо SIGTERM. Это исправляет воспроизведённую потерю свежего localStorage при остановке менеджером. Принудительное завершение остаётся отдельным действием; сохранность незаписанных данных при нём не гарантируется.
- На локальном стенде проверены localStorage и IndexedDB после штатного закрытия, последующего сбоя и восстановления снимка той же версии. Проверки не охватывают сбой питания или миграцию между версиями Chromium.
- Исправлен запуск изолированных Python-проверок релиза: соседние проверенные модули доступны без отключения изоляции и без подмены стандартной библиотеки.
- Ядро Chromium 153.0.8010.36 не изменилось. Разрыв с исправлениями безопасности .52/.53 сохраняется; кандидат не является security-current.

## Direct 0.7.0 (63) — опубликован

- Собрано ядро Chromium 153.0.8010.36 для Apple Silicon с Metal; проверены происхождение исходников, подпись ядра и соответствие встроенного runtime его отчёту. GUI A→B→A и проверки скачанных ZIP/DMG завершены; релиз опубликован на GitHub и browser.free.
- Версия ядра выбрана пользователем; она старее обновления безопасности .52/.53. Перенос этих исправлений не подтверждён; подпись и нотарификация не устраняют этот разрыв.
- Добавлены проверки совместимости данных профилей, разграничение настроенных и подтверждённых параметров диагностики, ограничения нагрузки сборки.
- Усилены проверки секретов, состава архивов и загрузки файлов. Developer ID, нотарификация Apple, stapling и Gatekeeper пройдены для точных опубликованных архивов.

# Direct 0.6.20 (62) — нативное управление разделами

- Добавлена нативная кнопка macOS для показа и скрытия папок и тегов.
- Состояние рабочей области сохраняется без потери выбранного рабочего места.
- Вторичные действия остаются в существующих меню; Chromium не пересобирался.

# Direct 0.6.19 (61) — обычный публичный релиз

- Сохранено компактное UX-улучшение массовой панели: область текущего списка видна сразу.
- Релизный тег возвращён к обычному формату без суффикса `-current`.
- ZIP и DMG публикуются вместе с checksum-файлами.
- Chromium не пересобирался.

# Direct 0.6.18 (60) — понятный выбор в массовых действиях

- В панели массовых действий явно указано, что выбрано в текущем списке.
- Пользовательский текст использует «рабочее место», а технический термин «профиль» скрыт.
- Без пересборки Chromium.

# Direct 0.6.17 (59) — спокойный sidebar

## Что изменилось

- Вторичные умные подборки скрыты до запроса и открываются одной строкой «Умные подборки».
- Папки и теги остаются доступными сразу, а длинные списки раскрываются отдельно.
- Основные действия профиля не меняются и не теряются.
- Проверена клавиатурная навигация и адаптивная вёрстка.
- Chromium не пересобирался; используется проверенное ядро 152.0.7977.82.

# Direct 0.6.16 (58) — компактная диагностика профиля

## Что изменилось

- Стабильность, события, расширения, cookies, snapshots и восстановление собраны в один скрытый раздел «Диагностика и восстановление».
- Основной экран профиля стал короче и показывает только ежедневные действия.
- Технические сведения и расчёт размера данных по-прежнему доступны по запросу.
- Проверена адаптивная вёрстка в узком и широком окне.
- Chromium не пересобирался; используется проверенное ядро 152.0.7977.82.

# Direct 0.6.15 (57) — удобный импорт прокси

## Что изменилось

- Строка импорта прокси больше не маскируется как пароль: вставленный адрес можно проверить глазами до импорта.
- Пароль прокси по-прежнему защищён SecureField и хранится отдельно в Keychain.
- Сохранены минималистичный интерфейс, подсказки и прежняя изоляция профилей.
- Chromium не пересобирался; используется проверенное ядро 152.0.7977.82.

# Direct 0.6.14 (56) — аккуратные поля и теги

## Что изменилось

- Поле стартовой страницы объясняет, когда используется адрес.
- Редактор тегов сообщает лимит доступным для VoiceOver текстом и подсказывает быстрый ввод через Return и запятую.
- Проверена адаптивная вёрстка профилей, редактора и панели тегов на узком и широком окне.
- Chromium не пересобирался; используется проверенное ядро 152.0.7977.82.

# Direct 0.6.13 (55) — компактный интерфейс и понятные действия профиля

## Что изменилось

- Подсказки у действий «Окно» и «Изменить» объясняют результат до нажатия.
- Подробности среды остаются скрытыми по умолчанию и получают явное объяснение, зачем их раскрывать.
- Проверены сценарии запуска, сухой прогон, адаптивная вёрстка и основные операции профилей.
- Chromium не пересобирался; используется проверенное ядро 152.0.7977.82.

# Direct 0.6.12 (54) — dry-run запуска и privacy-safe паспорт профиля

- добавлен UI-независимый `BrowserLaunchDryRun`: до запуска можно получить единый результат по runtime, локальным данным, прокси, профилю и процессу;
- dry-run не содержит пути, proxy endpoint, PID, URL, cookies или другой приватный вывод;
- добавлены состояния `ready`, `review` и `blocked`, чтобы интерфейс мог показать одну причину и следующее действие;
- добавлен регрессионный тест на отсутствие приватных данных в preview;
...
- выбор актуальной версии расширения стал однопроходным, без массива всей истории версий;
- добавлен регрессионный тест на раздутую storage-папку, чтобы диагностика оставалась быстрой и не читала приватные данные;
- release-аудит теперь завершает служебный процесс сразу после записи подписанного evidence, без зависания на UI lifecycle;
- Chromium не пересобирался: используется проверенный runtime 152.

## Direct 0.6.7 (49) — локальная проверка профиля

- в сведениях профиля появился privacy-safe блок локального хранилища: localStorage, sessionStorage, IndexedDB, service workers и cache проверяются только по факту наличия областей, без чтения URL, ключей и значений;
- инспектор профиля игнорирует symlink-записи внутри Chromium storage и не уходит за пределы текущего рабочего места;
- defensive checker matrix переведена ближе к тестовому стенду: storage namespace isolation теперь закрыт локальной проверяемой логикой;
- Chromium не пересобирался: используется проверенный runtime 152.

# Direct 0.6.6 (48) — быстрее и понятнее

- readiness-ошибки теперь показывают одну главную причину и конкретное действие;
- runtime-локатор разбирает метаданные встроенного Chromium один раз;
- сканер расширений выбирает новую версию без сортировки всей истории;
- журнал активности не пересортировывается при каждой записи;
- Chromium не пересобирался: используется проверенный runtime 152.

# Direct 0.6.5 (47) — простой старт профиля

- Главная кнопка создания открывает быстрый мастер «название + назначение».
- Расширенные настройки оставлены в дополнительном меню.
- При проверке среды показывается понятное состояние вместо немой блокировки.
- Chromium не пересобирался: используется прежний проверенный runtime 152.

## Direct 0.6.4 (46) — согласованная среда профиля

- CSS media queries и Screen API теперь получают одну и ту же геометрию профиля; устранено точное расхождение, найденное строгим аудитом A → B → A.
- Чтение и кодирование Canvas используют единое детерминированное преобразование, а цифровая тишина OfflineAudio остаётся тишиной.
- Отчёт выпуска перечисляет безопасные идентификаторы строгих несоответствий без URL, cookies, прокси и данных профиля.
- Проверка расширений читает фактический каталог Chromium `Default/Extensions`.
- Центр профиля отличает «настроено» от доказанной сетевой готовности, а повторяющаяся карточка проверки удалена.
- Проверка открытого репозитория корректно отличает резерв версии от полного контракта опубликованного архива и больше не падает на неполной записи.

## Direct 0.6.3 (45) — запуск без лишних блокировок

- Исправлено ошибочное блокирование запуска, когда macOS ещё не успела сообщить размер экрана.
- Профиль теперь использует безопасные значения среды по умолчанию и открывается сразу.
- Основная колонка каталога адаптируется к широкому окну без пустой правой области.

## Direct 0.6.1 (43) — корректная статистика

- Исправлена отправка версии и build в обезличенной статистике: при запуске без Info.plist больше не появляются `0.0.0 (0)`.
- Telegram-отчёты получают корректную редакцию, событие, количество профилей и профилей с прокси.

## Direct 0.6.1 (42) — цельный рабочий профиль

- Инспектор показывает контракт среды, историю стабильности, локальные события и риски расширений.
- Перед запуском автоматически создаётся атомарный snapshot; восстановление поддерживает rollback.
- Локальная резервная копия шифруется AES-GCM; данные прокси остаются в Keychain.
- Экономия памяти обратимо приостанавливает только управляемые процессы и возобновляет их перед остановкой.
- Командная палитра, чистый запуск, стартовые вкладки, clone-preview и импорт cookies объединены в единый рабочий поток.
- Privacy-safe диагностика и open-source проверки сохранены; URL, cookies, пароли и значения прокси не попадают в журнал.

## Direct 0.6.0 (39) — простой профиль, автоматическая проверка

- Быстрое создание рабочего места по названию и назначению: работа, исследование, тестирование или личное.
- Добавлены Identity Contract и единый центр готовности с понятными причинами проблем.
- Стабильность профиля хранится локально ограниченной историей; добавлены атомарные snapshots и проверка совместимости Chromium.
- Командная палитра, смысловой поиск по имени, заметкам, тегам и стартовому сайту, автоматические подсказки группировки.
- Чистый запуск, стартовый набор вкладок, компактный локальный журнал и объяснение fingerprint по запросу.
- Безопасный импорт cookies из JSON и Netscape с ограничением размера, количества и проверкой формата.
- Исправлено чтение истории стабильности с ISO-датами; все новые операции остаются локальными и не получают URL, cookies или секреты.

## Direct 0.5.8 (38) — обезличенная статистика

- Добавлен opt-out переключатель обезличенной статистики в настройках.
- Передаются только агрегированные события запусков, создания профилей и текущих счётчиков профилей и прокси.
- URL, cookies, имена и идентификаторы профилей, значения прокси и fingerprint-данные никогда не сериализуются.
- Сайт принимает события через privacy-thresholded API; Telegram-уведомления работают только при заданных серверных секретах.

## Direct 0.5.7 (37) — September 11, 2026

- В шапке каталога скрываются пустые оперативные фильтры, чтобы не перегружать ежедневный экран.
- Активный пустой фильтр остаётся видимым, поэтому из него всегда можно вернуться к «Все».

## Direct 0.5.6 (36) — September 11, 2026

- Каталог рабочих мест стал единственным стартовым экраном: поиск, папки, теги и действия находятся в одном месте.
- Убрана постоянная кнопка перехода на дублирующий Home; резервный маршрут сохранён в меню и горячей клавише.
- Пустой каталог сразу предлагает создать первое рабочее место без промежуточного экрана.
