...
- Запуск разделён на пять проверяемых этапов: runtime, storage, proxy,
  consistency и process. Ошибка теперь указывает точный этап, а запуск
  блокируется при недоступной папке данных или менее 1 ГиБ свободного места.
- Жизненный цикл профиля различает запуск, штатное закрытие, ожидание и
  отдельную подтверждаемую принудительную остановку. Появилась компактная
  полоса активных профилей с Focus/Stop/elapsed time; SIGKILL больше не
...
- NeAntik записывает минимальное локальное доказательство неожиданного
  ...
  ограничено до 12 управляемых запусков, а переходы процесса покрыты
  детерминированными тестами.
- Запущенный профиль можно редактировать с явным предупреждением, что новые
  параметры применятся при следующем запуске. «Дублировать» переименовано в
  «Создать похожий» с пояснением новой сессии и идентичности.
- Массовая проверка прокси показывает успешные и ошибочные результаты и умеет
  повторить только ошибки. Форматы прокси стали короче, а подробности проверки
  и авторизации собраны в один раскрываемый блок.
- Добавлены бюджеты размеров Swift-файлов, измерения cold/warm готовности Dev
  manager и отдельный sampling CPU/RSS процесса менеджера без Chromium.
- Добавлен нативный «Центр готовности»: он одной локальной проверкой показывает
  приложение, встроенный Chromium, доступность общей папки данных, состояния
  процессов и подключений. Экран прямо объясняет, что в разрешениях macOS
  нужно выбирать основной `NeAntik.app`, умеет повторить проверку без
  перезапуска и безопасно скопировать ограниченную диагностику без паролей,
  прокси-адресов, IP, seed, хэшей и путей отдельных профилей.
- Добавлен детерминированный аудит установленного размера с отдельными
  бюджетами manager/runtime/compliance/other. Он фиксирует фактическую причину
  веса приложения — встроенный Chromium — и не позволяет маскировать рост
  нативного интерфейса ростом runtime. Консолидированный план из 100
  улучшений и проверок записан в `docs/NEANTIK_V4_READINESS_PLAN.md`.
- Командная строка workspace объединяет поиск, действия, фильтры и создание,
  а вычисляемые представления «Все», «Запущены», «Внимание» и «Без запусков» сразу
  показывают количество профилей без новой схемы хранения или сетевых
  запросов.
- Добавлены сессионные плотности строк «Удобно»/«Компактно», контекстные
  пустые состояния и отдельное действие заметки в меню профиля. Редактор явно
  предупреждает, что прямое подключение раскрывает обычный публичный адрес Mac
  или системного VPN.
- Исследованы актуальные рабочие сценарии Vision, GoLogin, Octo, Dolphin,
  AdsPower, Multilogin, Incogniton, Kameleo, MoreLogin и BitBrowser; 75
  проверенных кандидатов и границы минималистичного v4 workspace записаны в
  `docs/NEANTIK_V4_WORKSPACE.md`.
...
- На широком окне строка профиля показывает в адаптивных колонках запуск,
  имя и организацию, статус, подключение, заметку и последний запуск; при
  минимальном размере она автоматически переходит в компактный вид.
- У каждой строки появилось видимое меню действий, а создание и запуск
  получили единый зелёный акцент. Счётчик в заголовке учитывает только
  действительно работающие процессы, не проверку готовности движка.
- Публичные бинарники и ссылки остаются на `0.3.21 (24)`. Изменение не
  добавляет зависимости, изображения, хранилища, фоновые службы или Chromium.
- Direct release evidence теперь строго прекращается при любой ошибке Secure
  Enclave: production-код больше не создаёт экспортируемый software-key, а
  release gate отдельно запрещает App Sandbox и проверяет update metadata
  именно упакованного кандидата.
- Удалённый профиль сразу исчезает из всех локальных UI-состояний даже при
  отложенной очистке Keychain; временная проверка процессов больше не
  учитывается как запущенный браузер, а строки списка получили явные подписи
  профиля, статуса и подключения для VoiceOver.

## Direct 0.3.22 (25) — August 30, 2026
