...
| Keyboard and local UI preferences | `ProfileCommands.swift`, `NeAntikShortcutCatalog.swift`, `WorkspacePreferenceStore.swift`, `NeAntikSettingsView.swift` | Fixed menu-backed shortcuts, modal-safe focused commands and density-only local persistence; transient one-shot shortcut-reference request scrolls/focuses Settings search without saving navigation state; no global hooks or hidden destructive shortcuts |
