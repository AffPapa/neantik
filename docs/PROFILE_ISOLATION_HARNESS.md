...
companion local filesystem/process smoke harness: it creates isolated
temporary BrowserData and lock stores, launches separate OS child processes
that hold advisory locks, forces abrupt termination, and runs a separate
recovery lock acquisition. It never launches Chromium,
...
harness. It demonstrates separate temporary filesystem inodes, real advisory
lock contention, abrupt child termination, and a new recovery lock acquisition.
...
BrowserData semantics, or long-lived resource behavior. A future authorized
