...
- manager-only кандидат `0.7.11 (74)` уточняет VoiceOver-контекст preview;
...
Последний Doctor проход 24 сентября 2026 остановился на отдельном
`deploy credential is unavailable` для AffPapa workflow. Это не отменяет уже
проверенные GitHub Releases и browser.free; перед будущей публикацией через
AffPapa требуется восстановить этот credential.
